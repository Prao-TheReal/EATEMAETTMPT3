...
using System.Numerics; // We keep this for Matrix4x4, but Vector3 will use your custom struct
using System.Text;
    ...
    public List<CharacterData> GetAllCharacters(CameraData camera)
    {
        ...
        try
        {
            ...
            for (int i = 0; i < charCount; i++)
            {
                ...
                if (Math.Abs(location.X) < 0.01f && Math.Abs(location.Y) < 0.01f && Math.Abs(location.Z) < 0.01f) continue;

                // [FIXED] Using Instance Method for Custom Vector3
                double distDouble = location.Distance(camera.Location) / 100.0;
                float dist = (float)distDouble;

                bool isMe = isAddressMatch || (dist < 2.5f);

                var newChar = new CharacterData { Address = charAddr, Location = location, IsPlayer = isMe, Distance = dist };

                if (!newChar.IsPlayer && dist < 150)
                {
                    // 1. Get Bones (and trigger name mapping!)
                    newChar.Bones = GetSkeleton(charAddr);

                    // 2. Get True Weakspot using the mapped names
